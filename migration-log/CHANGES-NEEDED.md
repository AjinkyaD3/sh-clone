...
### To do — new, from user, not investigated yet
- [ ] **Header sticky behavior needs review** — user flagged that the header is sticky on many pages and asked to look at how it actually works/behaves. Not investigated yet — this is a bare note per user's explicit "just note this down for now" request, no root-cause check or fix attempted. When picked up: check the actual sticky-header implementation (`components/Header.tsx` + whatever CSS/JS drives the sticky behavior — likely another Avada feature that may or may not be correctly wired up in this static migration, same category as several other JS-driven Avada widgets already found broken this project) across a sample of pages, not just assume it's fine or assume it's broken.
