...
- [ ] **"Scroll section" cards rendering squished/broken sitewide.** Found while spot-checking image parity on 5 pages: Panic Room Doors, Communal Entrance Doors, Security Aluminium Windows, Colllabsible Grilles, Tracless Garage Doors, and the Grilles/Shutters hub all use Avada's `fusion-scroll-section awb-swiper-full-sections` widget, which needs each `.swiper-slide` full-width — but the site's global `.swiper-wrapper { display: grid; grid-template-columns: repeat(auto-fit, minmax(250px,1fr)) }` fallback (added earlier for a different, ordinary carousel widget) squeezes these into ~250px columns, so every card on these pages renders as a thin, overlapping, unreadable vertical sliver. Root cause confirmed via DOM/computed-style inspection: `.fusion-scroll-section.awb-swiper-full-sections .swiper-wrapper` is inheriting the generic grid rule. **A fix was written and verified on the dev server across all 5 pages** (scoped override: wrapper → `display: block`, slides → `width: 100%`, same pattern as the existing homepage-marquee override) **but was reverted** — the user canceled the Vercel deploy before it finished and asked to undo the commit rather than wait/retry, so production is unaffected and still broken. The correct fix is documented in commit `598b741` (now reverted by `f8ae692`) if picked back up.
