...
### Fixed — 2026-09-11 (same day, user asked to fix immediately)
- [x] **"Scroll section" cards rendering squished/broken sitewide — FIXED.** Found while spot-checking image parity on 5 pages at the user's request (Panic Room Doors, Communal Entrance Doors, Security Aluminium Windows, Colllabsible Grilles, plus Tracless Garage Doors and the Grilles/Shutters hub via a codebase-wide search). All use Avada's `fusion-scroll-section awb-swiper-full-sections` widget, which needs each `.swiper-slide` full-width — but the site's global `.swiper-wrapper { display: grid; grid-template-columns: repeat(auto-fit, minmax(250px,1fr)) }` fallback (added earlier for a different, ordinary carousel widget) was squeezing these into ~250px columns, causing all cards on a page to render as thin overlapping vertical slivers with unreadable text. Root cause confirmed via DOM/computed-style inspection before touching anything. Fix: scoped override in `app/layout.tsx` — `.fusion-scroll-section.awb-swiper-full-sections .swiper-wrapper` → `display: block`, `.swiper-slide` → `width: 100%` with spacing between — same "higher specificity than the generic rule" pattern already used for the homepage logo marquee. Verified visually on all 5 affected pages on the dev server post-fix: cards now stack full-width and readable, matching the live WordPress site's behavior. `tsc` clean. Not yet deployed to production — local fix only until user confirms and this is pushed.
