...
## Confirmed bug, root-caused — needs a decision on which copy to keep, not a straightforward "delete the dupe"

### Sectional Garage Doors (`/garage-doors/sectional-garage-doors`)
The 4 product cards (R40 Sectional / TL Sectional / SHD Side Hinged Doors / SSD Side Sliding Doors) appear **twice**. Investigated and confirmed: **this exists in `content.html` itself**, not introduced by the JSX conversion — the converter parses it twice straight from the raw scraped HTML, so the old (`dangerouslySetInnerHTML`) page already has this exact same duplication today. Structure: a mobile-only copy (wrapped in a row with `fusion-no-medium-visibility fusion-no-large-visibility`, i.e. hidden on tablet/desktop) sits right next to a second, unrestricted copy of the identical 4 cards (rows 15–18, re-declared starting around line 2554 of `app/new/garage-doors/sectional-garage-doors/page.tsx`) that shows on *every* viewport. This pattern strongly suggests the "restored 4 product-variant cards" fix from an earlier migration session (`PROGRESS.md`, "Completed (continued, 4)" — cards were manually re-inserted from the live site because the original scrape was missing them) accidentally duplicated content that was already partially present via the mobile-only row. The live site currently shows neither copy in its extracted text. **Needs a decision**: which copy (if either) should survive, since content.html is the shared source for both the old and new page — fixing it in one place fixes both.
